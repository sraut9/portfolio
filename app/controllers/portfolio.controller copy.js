...
exports.findAll = (req, res) => {
  ...
  Portfolio.find(condition)
    .then(data => {
      ...
      // retrieve latest nav from URL
      var i=0;
      // for (i=0; i <= 2; i++) {
        //console.log("i: " + i);
        axios.get(url[i])
        .then(response => {
            data[i].last_nav_price = getData(response.data);
            //console.log("portfolio findAll " +i+": " + data[i]);
          })
        .catch(error => {
          //console.log("i, url: " + i + ", " + url[i]);
          console.log(error);
        })
      // }
      axios.get(url[++i])
      ...
      .then(response => {
          data[i].last_nav_price = getData(response.data);
          //console.log("portfolio findAll" + data);
          res.send(data);
        })
      .catch(error => {
        //console.log("i, url: " + i + ", " + url[i]);
